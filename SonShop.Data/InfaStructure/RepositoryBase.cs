...
namespace SonShop.Data.InfaStructure
{
    public abstract class RepositoryBase<T> where T : class
    {
        ...
        public virtual IEnumerable<T> GetMultiPaging(Expression<Func<T,bool>> predicate, out int total, int index = 0, int size = 0, string[] includes = null)
        {
            int skipCount = index * size;
            IQueryable<T> _resetSet;

            //HANDLLE INCLUDES FOR ASSOCIATED OBJJECT IF APPLCABLE
            if(includes != null && includes.Count() > 0)
            {
                var query = dataContext.Set<T>().Include(includes.First());
                foreach (var include in includes.Skip(1))
